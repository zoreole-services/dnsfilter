...
| Mode | Description |
|------|--------------|
| **BINDLOCAL** | The solution directly generates an RPZ (Response Policy Zone) file, which is locally read by the BIND process on the **host**. |
| **BINDTRANSFER** | The solution generates the RPZ file and serves it using a BIND instance **running inside the container**, acting as the **master server** for the RPZ zone that can be queried by BIND slave servers. |
...
Those variables should be configured on env file located at env/dnsfilter.env
...
The RPZ file generated by the script is read directly by a **BIND instance running on the host**.

The following volume must be uncommented/added in your docker-compose.yaml file, in order to propagate the generated RPZ data into the local BIND server:
```
volumes:
  - /etc/bind/dnsfilter_zones:/usr/src/app/zones
```
...
In this mode, BIND runs **inside the container**, acting as **master** for the RPZ zone.  
Other DNS servers can perform zone transfers from this master.

This variable should be configured on env file located at env/bind.env
